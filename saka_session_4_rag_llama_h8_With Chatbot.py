...
def generate(state: State):
    ...
    if not docs_content:  # Handle empty context
...
def gradio_chat(message, history):
    print(f"Vector store in gradio_chat: {vector_store}") # Debugging print
    # The `retrieve` function expects a state dictionary with a 'question' key and the vector store
    retrieve_state = {"question": message}
    # Call the retrieve function to get the context, passing the vector_store
    retrieved_state = retrieve(retrieve_state, vector_store)
    print(f"Retrieved state: {retrieved_state}") # Debugging print
    print(f"Retrieved context: {retrieved_state.get('context') if retrieved_state else None}") # Debugging print

    if retrieved_state and "context" in retrieved_state:
        # The `generate` function expects a state dictionary with 'question' and 'context'
        generate_state = {"question": message, "context": retrieved_state["context"]}
        # Call the existing generate function
        result = generate(generate_state)
        # Return the answer for Gradio
        return result["answer"]
    else:
        return "Could not retrieve relevant context."


gr.ChatInterface(gradio_chat).launch(share=True)